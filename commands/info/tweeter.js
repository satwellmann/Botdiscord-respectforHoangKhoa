...
module.exports = {
  ...
  run: async (client, msg, args) => {
    let user = args[0]
    if(!user) return msg.channel.send("Cung cấp tên twitter của bạn")
    ...
    try {
      ...
      if(e.status === 403) return msg.channel.send("Chế độ riêng tư của người dùng này hoặc tài khoản đã bị xóa")
      else if(e.status === 404) return msg.channel.send("Không tìm thấy")
      else return msg.channel.send(`Lỗi không thể nhận biết: \`${e.message}\``)
    }
  }
}