...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        else return message.channel.send("**:x: | Trò chơi đã bắt đầu..**");
        ...
        collector.on("collect", async (reaction, user) => {
            ...
            if (aki.progress >= 70 || aki.currentStep >= 78) {
                ...
                message.channel.send(new MessageEmbed()
                    .setTitle("Đây có phải là nhân vật của bạn không?")
                    ...
                    .then(collected => {
                        ...
                        if (content == "y" || content == "yes")
                            return message.channel.send(new MessageEmbed()
                                .setColor("RANDOM")
                                .setTitle("Tuyệt vời! Đã đoán đúng một lần nữa.")
                                .setDescription("Tôi thích chơi với bạn!"));
                        else
                            return message.channel.send(new MessageEmbed()
                                .setColor("RANDOM")
                                .setTitle("Ờ. bạn đã chiến thắng")
                                .setDescription("Tôi thích chơi với bạn!"));
                    });
                return;
            }
            msg.edit(new MessageEmbed()
                .setTitle(`${message.author.username}, Câu hỏi ${aki.currentStep + 1}`)
                .setColor("RANDOM")
                .setDescription(`**${aki.question}**\n${aki.answers.map((x, i) => `${x} | ${emojis[i]}`).join("\n")}`));
        });


        collector.on("kết thúc", () => {
