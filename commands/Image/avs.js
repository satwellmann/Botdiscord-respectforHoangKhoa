...
module.exports = {
    ...
    run: async (bot, message, args) => {
        if (!message.guild.me.hasPermission('ATTACH_FILES')) return message.channel.send("**Thiếu quyền - [ATTACH_FILES]!**");
        if (!args[0]) return message.channel.send("**Người dùng nào bạn muốn trở thành cơ sở?:>>**");
        if (!args[1]) return message.channel.send("**Người dùng nào bạn muốn trở thành cơ sở?:>>**");
        ...
        if (!base) return message.channel.send("**Không tìm thấy người dùng cơ sở!**");
        ...
        if (!overlay) return message.channel.send("**Không tìm thấy người dùng lớp phủ .-.!**");
        ...
        try {
            ...
            return message.channel.send(`Ồ không, đã xảy ra lỗi: \`${err.message}\`. Vui lòng thử lại sau!`);
        };
    }
};